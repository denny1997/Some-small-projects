...
let boxheight:CGFloat = 0.05 //定义方块的厚度
let boxLengthWidth:CGFloat = 0.4 //定义方块的边长
let actionOffet:Float = 0.6 //定义方块起始位置
var actionSpeed:Float = 0.005 //定义方块的运动速度
...
class ViewController: UIViewController {
    ...
    var direction = true //确定方块的移动的方向
    var height = 0
    var score = 0
    
    
    var previousSize = SCNVector3(boxLengthWidth, boxheight, boxLengthWidth) //确定方块初始大小
    var previousPosition = SCNVector3(0, boxheight*0.5, 0) //初始位置
    ...
    var sounds = [String: SCNAudioSource]() //音效
    
    var highestScore = [highScore]() //最高分记录

    override func viewDidLoad() {
        super.viewDidLoad()
        playButton.isHidden = true
        hardness.isHidden = true //显示按钮
    ...
    @IBAction func handleTap(_ sender: Any) {
        if let currentBoxNode = gameNode?.childNode(withName: "Block\(height)", recursively: false) {
            ...
            //分数统计
            //scoreLabel.text = "Score:\(height+1)"
...
extension ViewController {
    ...
    func gameOver() {
        ...
        //保存最高分信息
        if score > savedScore?[0].score ?? 0 {
            guard let score = highScore(score: score) else {
...
extension ViewController:ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        if let planeAnchor = anchor as? ARPlaneAnchor,node.childNodes.count < 1,updateCount < 1 {
            print("捕捉到平地")
            //2.创建一个平面    （系统捕捉到的平地是一个不规则大小的长方形，这里将其变成一个长方形）
